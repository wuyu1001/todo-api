...
@apis.route('/file', methods=['GET', 'POST'])
...
if (!getUrlInfo()) { return };
            ...
            var cf_openCrowdWindow = function () {
                ...
                var index = layer.open({
                    ...
                    content: '<div>' +
                        ...
                        + '     <label class="layui-form-label layui-btn" id="lb_selectDate">∑∂Œß—°‘Ò</label>'
                        + '       <div class="layui-input-inline">'
                        + '            <input class="layui-input" placeholder="ø™ º»’" id="LAY_rqrange_s">'
                        + '       </div>'
                        + '        <div class="layui-input-inline">'
                        + '            <input class="layui-input" placeholder="Ωÿ÷π»’" id="LAY_rqrange_e"> '
                        + '        </div>'
                        + '        <div class="layui-input-inline">'
                        + '             <button class="layui-btn layui-btn-primary" id="bt_getrenqunRpt">¿≠»° ˝æ›</button>'
                        + '        </div>'
                        + '             <div><div class="layui-btn-group">'
                        + '<button class="layui-btn layui-btn-normal" id="bt_getrenqunNums">±Í«©¥ ∆µ</button>'
                        + '<button id="bt_editDiscount" class="layui-btn">–ﬁ∏ƒ“Áº€</button>'
                        + '<button id="bt_RqSart" class="layui-btn">≤Œ”ÎÕ∆π„</button>'
                        + '<button id="bt_RqStop" class="layui-btn">‘›Õ£Õ∆π„</button>'
                        + '<button id="bt_RqDelete" class="layui-btn">…æ≥˝</button>'
                        + '<button class="layui-btn layui-btn-danger" id="bt_TargetingYun">‘∆»À»∫∞¸</button>'
                        + '</div></div>'
                        + '    </div>'
                        + '</div>'
                ...
                var start = {
                    ...
                    choose: function (datas) {
                        end.min = datas; //ø™ º»’—°∫√∫Û£¨÷ÿ÷√Ω· ¯»’µƒ◊Ó–°»’∆⁄
                        end.start = datas //Ω´Ω· ¯»’µƒ≥ı º÷µ…Ë∂®Œ™ø™ º»’
                    }
                };
                var end = {
                    min: laydate.now(-30),
                ...
                $("#lb_selectDate").click(function () {
                    var strSUI = '<button class="layui-btn layui-btn-mini layui-btn-primary datebt" data=0>ΩÒÃÏ</button>'
                        + '<button class="layui-btn layui-btn-mini layui-btn-primary datebt" data=-1>◊ÚÃÏ</button>'
                        + '<button class="layui-btn layui-btn-mini layui-btn-primary datebt" data=-2>π˝»•2ÃÏ</button>'
                        + '<button class="layui-btn layui-btn-mini layui-btn-primary datebt" data=-3>π˝»•3ÃÏ</button>'
                        + '<button class="layui-btn layui-btn-mini layui-btn-primary datebt" data=-7>π˝»•7ÃÏ</button>'
                        + '<button class="layui-btn layui-btn-mini layui-btn-primary datebt" data=-14>π˝»•14ÃÏ</button>'
                        + '<button class="layui-btn layui-btn-mini layui-btn-primary datebt" data=-30>π˝»•30ÃÏ</button>';
                ...
                document.getElementById('bt_getrenqunRpt').onclick = function () {
                    var cf_do = function () {
                        ...
                        if (startDate == theDate) {
                            ...
                            if (startDate > endDate) {
                                layer.msg("ø™ º»’∆⁄≤ªƒ‹¥Û”⁄Ω· ¯»’∆⁄£°«Î÷ÿ–¬—°‘Ò£°");
                                layer.close(index);
                                return;
                            } else {
                                if (endDate >= theDate) {
                                    layer.msg("±®±Ì ˝æ›≤ªƒ‹”⁄ µ ±(ΩÒ»’)Õ¨ ±¿≠»°£°«Î…Ë÷√Ωÿ÷π»’∆⁄‘Á”⁄ΩÒ»’£°");
                                    layer.close(index);
                                    return;
                                };
                ...
                $("#bt_getrenqunNums").click(function () {
                    var cf_do = function () {
                        ...
                        var msgHtml = "";
                        for (var p in fres) { msgHtml = msgHtml + " Ù–‘°æ" + p + "°ø≥ˆœ÷¥Œ ˝£∫" + fres[p] + " °£</br>" };
    ...
    if (!getUserRank(2,UserRank)) {
        layer.msg("≈˙¡øµ˜’˚»À»∫∞¸±Í«©π¶ƒ‹Ωˆ∂‘ ⁄»®”√ªßø™∑≈£°<br/>–Ë“™∂‘µ±«∞µÍ∆Ã£®" + User.nickName + "£© ⁄»®£°<br/> «Î¡™œµΩª¡˜»∫π‹¿Ì ⁄»®£°");
        return;
    };
    */
                        layer.prompt({ title: " ‰»Î–ﬁ∏ƒµƒ“Áº€[5-300]÷Æº‰£¨≤¢»∑»œ", formType: 0 }, function (nds, index) {
                            layer.close(index);
                            var sIds = getjqGridSelarrrow("#list2");
                            if (sIds.length < 1) {
                                layer.msg("Œ¥—°÷–»Œ∫Œ»À»∫∞¸±Í«©£°");
                            ...
                            if (int_nds < 5 || int_nds > 300) {
                                layer.msg(" ‰»Îµƒ“Áº€≤ª‘⁄‘ –Ì∑∂Œß£¨«Î ‰»Î5-300µƒ’˚ ˝£°");
                                return;
                            };
                            var targetings = new Array();
                            for (var i = 0; i < sIds.length; i++) {
                            ...
                            if (cf_adgroupTargetingUpdate(targetings, User)) {
                                ...
                                layer.msg("∏¸–¬ ˝æ› ß∞‹£ª«ÎÀ¢–¬“≥√Ê∫Û÷ÿ ‘£°"); return;
                            };
                        });
    ...
    */
                        ...
                        var cf_changeCrowdOnlineStatus = function (sIds, Onlinestatus) {
                            ...
                            if (cf_adgroupTargetingUpdate(targetings, User)) {
                                ...
                                layer.msg("Õ∆π„◊¥Ã¨“—∏¸–¬£°");
                                return;
                            } else { layer.msg("∏¸–¬ ˝æ› ß∞‹£ª«ÎÀ¢–¬“≥√Ê∫Û÷ÿ ‘£°"); return; };
    ...
    */
	...
	return;
		...
		*/
                        ...
                        layer.confirm("»∑∂®“™…æ≥˝’‚–©»À»∫∞¸ ˝æ›?“ªµ©…æ≥˝£¨À˘”– ˝æ›Ω´Œﬁ∑®ª÷∏¥£°", { icon: 3, title: "»∑»œ…æ≥˝" }, function (index) {
                            layer.close(index);
                ...
                $("#bt_TargetingYun").click(function () {
                    var cf_do = function () {
                        ...
                        var itemId = AdgroupInfo.outsideItemNumId; if (itemId != "") { } else { layer.msg("ªÒ»°±¶±¥ ˝æ› ß∞‹£¨«Î÷ÿ–¬µ«¬Ω£°"); return };

                        var strUI = '<div style="padding: 15px; line-height: 22px; background-color: #fff; color: #000;">' + '<div id="uaList" style="height:245px">'
                            + '<span id="ua01">'
                            + '«‡…´∑÷œﬂ <button class="layui-btn layui-btn-mini layui-btn-normal">”¶”√µΩµ±«∞º∆ªÆ</button><button class="layui-btn layui-btn-mini layui-btn-danger">…æ≥˝</button>'
                            + '<hr class="layui-bg-cyan">'
                            + '</span>'
                            + '<span id="ua02">'
                            + '«‡…´∑÷∏Óœﬂ <button class="layui-btn layui-btn-mini layui-btn-normal">”¶”√µΩµ±«∞º∆ªÆ</button><button class="layui-btn layui-btn-mini layui-btn-danger">…æ≥˝</button>'
                            + '<hr class="layui-bg-cyan">' + '</span>' + '</div>' + '<button class="layui-btn layui-btn-small layui-btn-normal" id="BakUserCrowdBt">±∏∑›»À»∫∞¸</button>'
                            + '</div>';
                        layer.open({
                            ...
                            success: function (layero, index) {
                                ...
                                //User.nickName = "›∏Ã‘µÁ◊”…ÃŒÒπ´Àæ";
                                //User.operName = "›∏Ã‘µÁ◊”…ÃŒÒπ´Àæ";
                                $.extend(postData, User);
                            ...
                            }, title: ['»À»∫∞¸‘∆π¶ƒ‹', 'font-size:18px;']
                        });
                        $("#BakUserCrowdBt").click(function () {
                            layer.prompt({ formType: 0, title: '«Î ‰»Î±∏∑›√˚≥∆' }, function (value, index, elem) {
                ...
                var cf_pageInitCrowd = function (w, h) {
                    jQuery("#list2").jqGrid({
                        datatype: "local",
                        //«Î«Û ˝æ›∑µªÿµƒ¿‡–Õ°£ø…—°json,xml,txt
                        colNames: ['◊¥Ã¨', 'À—À˜Õ∆π„', '“Áº€', '’πœ÷¡ø', 'µ„ª˜¡ø', 'µ„ª˜¬ ', 'ª®∑—', 'CPC',
                            '◊‹≥…ΩªΩ∂Ó', 'ROI', 'CVR', '◊‹≥…Ωª ˝', '◊‹π∫ŒÔ≥µ ˝', '◊‹ ’≤ÿ ˝',
                            'cid', 'iid', '«ß¥Œ’πœ÷ª®∑—'],//jqGridµƒ¡–œ‘ æ√˚◊÷
                        colModel: [
                            //jqGrid√ø“ª¡–µƒ≈‰÷√–≈œ¢°£∞¸¿®√˚◊÷£¨À˜“˝£¨øÌ∂»,∂‘∆Î∑Ω Ω.....
                            { name: 'onlineStatus', index: 'onlineStatus', width: 80, stype: 'select', formatter: 'select', editoptions: { value: ":All;0:‘›Õ£;1:Õ∆π„÷–" }, frozen: true },
                        ...
                        rowNum: 100,
                        //“ª“≥œ‘ æ∂‡…ŸÃı
                        rowList: [20, 50, 100, 200],//ø…π©”√ªß—°‘Ò“ª“≥œ‘ æ∂‡…ŸÃı
                        pager: '#pager2',//±Ì∏Ò“≥Ω≈µƒ’ºŒª∑˚(“ª∞„ «div)µƒid
                        sortname: 'id',//≥ı ºªØµƒ ±∫Ú≈≈–Úµƒ◊÷∂Œ
                        sortorder: "desc",//≈≈–Ú∑Ω Ω,ø…—°desc,asc
                        viewrecords: true, //∂®“Â «∑Ò“™œ‘ æ◊‹º«¬º ˝
                        multiselect: true, //‘ –Ì∂‡—°
                        loadonce: true, //øÕ∑˛∂À÷¥––
                        height: (h - 235),  //∂®“Â±Ì∏Ò∏ﬂ∂»
                        width: (w - 5),
                        //autowidth: true,
                        rownumbers: true,
                        shrinkToFit: false,
                    ...
                    jQuery("#list2").jqGrid('setGroupHeaders', {
                        ...
                        groupHeaders: [
                            { startColumnName: 'impression', numberOfColumns: 14, titleText: '±®±Ì ˝æ›' }
                        ]
                    });
